...
/// Download seletivo de um shader via rota /shaders/:filename do painel.
#[tauri::command]
pub async fn download_shader(
    app: tauri::AppHandle,
    filename: String,
) -> Result<Vec<u8>, String> {
    // Sanitiza: mantém só o nome do arquivo (sem path).
    ...
    let base = std::env::var("LAUNCHER_API_BASE")
        .unwrap_or_else(|_| "https://servermine.centralchamados.xyz".to_string());
    // O painel serve os shaders em /shaders/<filename>.
    let url = format!("{}/shaders/{}", base.trim_end_matches('/'), safe);
